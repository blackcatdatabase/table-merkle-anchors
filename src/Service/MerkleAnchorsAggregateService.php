...
namespace BlackCat\Database\Packages\MerkleAnchors\Service;

use BlackCat\Core\Database;
use BlackCat\Database\Packages\MerkleAnchors\Dto\MerkleAnchorDto;
use BlackCat\Database\Packages\MerkleAnchors\Mapper\MerkleAnchorDtoMapper;
use BlackCat\Database\Packages\MerkleAnchors\Repository\MerkleAnchorRepository;
 ...
 */
final class MerkleAnchorsAggregateService
{
    use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private MerkleAnchorRepository $merkleAnchorRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'merkle_anchors' => $this->merkleAnchorRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if (!$row) {
                ...
                throw new \BlackCat\Database\Packages\MerkleAnchors\ModuleException("Record {$idStr} not found for locking.");
            }

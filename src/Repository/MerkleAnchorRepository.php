...
namespace BlackCat\Database\Packages\MerkleAnchors\Repository;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\MerkleAnchors\Definitions;
use BlackCat\Database\Packages\MerkleAnchors\Criteria;
use BlackCat\Database\Packages\MerkleAnchors\Dto\MerkleAnchorDto as Dto;
use BlackCat\Database\Packages\MerkleAnchors\Mapper\MerkleAnchorDtoMapper as RowMapper;
...
final class MerkleAnchorRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\MerkleAnchors\Definitions::class; }
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [ 'merkle_root_id', 'anchor_type', 'anchor_ref' ] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\MerkleAnchors\Definitions::table());
            return $bulk->upsertMany($rows, [ 'merkle_root_id', 'anchor_type', 'anchor_ref' ], [ 'anchored_at', 'meta' ]);
        }
    ...
    */
    ...
    public function getByMerkleRootIdAndAnchorTypeAndAnchorRef(int $merkleRootId, string $anchorType, string $anchorRef, bool $asDto = false): array|\BlackCat\Database\Packages\MerkleAnchors\Dto\MerkleAnchorDto|null {
        $row = $this->getByUnique([ 'merkle_root_id' => $merkleRootId, 'anchor_type' => $anchorType, 'anchor_ref' => $anchorRef ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\MerkleAnchors\Mapper\MerkleAnchorDtoMapper::fromRow($row);
    }
    ...
    public function getIdByMerkleRootIdAndAnchorTypeAndAnchorRef(int $merkleRootId, string $anchorType, string $anchorRef) {
        $row = $this->getByMerkleRootIdAndAnchorTypeAndAnchorRef($merkleRootId, $anchorType, $anchorRef);
        return $row ? ($row['id'] ?? null) : null;
    }
    /** @return array<string,mixed>|\BlackCat\Database\Packages\MerkleAnchors\Dto\MerkleAnchorDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\MerkleAnchors\Dto\MerkleAnchorDto|null {

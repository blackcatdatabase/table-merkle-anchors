...
use BlackCat\Database\Packages\MerkleAnchors\Repository\MerkleAnchorRepositoryInterface;
...
class MerkleAnchorRepository implements MerkleAnchorRepositoryInterface, RepoContract, KeysetRepoContract
{
use OrderByTools, PkTools, RepositoryHelpers;

    /** @var mixed literal token for upsert keys (array or empty). */
    private mixed $tokenUpsertKeys = [ 'merkle_root_id', 'anchor_type', 'anchor_ref' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if ($helperKeys && class_exists(\BlackCat\Database\Support\BulkUpsertHelper::class)) {
          $bulk = new \BlackCat\Database\Support\BulkUpsertHelper($this->db, \BlackCat\Database\Packages\MerkleAnchors\Definitions::class);
          $bulk->upsertMany($rows, $helperKeys, [ 'anchored_at', 'meta' ]);
          return count($rows);
        }
